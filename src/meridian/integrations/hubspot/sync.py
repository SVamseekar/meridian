...
from meridian.integrations.hubspot.oauth import HubSpotClient
...
async def sync_tenant_inbound(
    ...
    logger.info('{"event": "hubspot_sync_started", "tenant_id": "%s"}', tenant_id)
    credentials = await get_hubspot_credentials(session, tenant_id)
    if credentials is None:
        logger.info(
            '{"event": "hubspot_sync_skipped_not_connected", "tenant_id": "%s"}', tenant_id
        )
    ...
    try:
        ...
        for _ in range(MAX_SYNC_PAGES):
            ...
            if after_company in seen_company_cursors:
                logger.warning(
                    '{"event": "hubspot_pagination_repeated_cursor", "entity": "company", "tenant_id": "%s"}',
                    tenant_id,
                )
                break
            ...
            logger.warning(
                '{"event": "hubspot_pagination_page_cap_hit", "entity": "company", "tenant_id": "%s", "cap": %d}',
                tenant_id,
                MAX_SYNC_PAGES,
            )
        ...
        for _ in range(MAX_SYNC_PAGES):
            ...
            if after_deal in seen_deal_cursors:
                logger.warning(
                    '{"event": "hubspot_pagination_repeated_cursor", "entity": "deal", "tenant_id": "%s"}',
                    tenant_id,
                )
                break
            ...
            logger.warning(
                '{"event": "hubspot_pagination_page_cap_hit", "entity": "deal", "tenant_id": "%s", "cap": %d}',
                tenant_id,
                MAX_SYNC_PAGES,
            )
        ...
        logger.info(
            '{"event": "hubspot_sync_completed", "tenant_id": "%s", "companies_upserted": %d, "deals_upserted": %d}',
            tenant_id,
            companies_upserted,
            deals_upserted,
        )
        ...
        logger.error(
            '{"event": "hubspot_sync_failed", "tenant_id": "%s", "error": "%s"}',
            tenant_id,
            exc,
            exc_info=True,
        )
        # Record the failure BEFORE rolling back: record_hubspot_sync_result
        # re-queries the credentials row and commits its own write, and
        # calling session.rollback() first leaves that later query unable to
        # see rows committed earlier in this same session (observed: a
        # credentials row committed before this sync pass started becomes
        # unreadable via this session after rollback(), not just this pass's
        # own uncommitted company/deal upserts). Discard the partial
        # company/deal upserts afterward, once the failure is durably
        # recorded.
