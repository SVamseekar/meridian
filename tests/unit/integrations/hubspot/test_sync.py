...
async def test_sync_tenant_inbound_success_with_mock_client(monkeypatch):
    ...
    async def mock_record_result(session, tid, status, error=None, commit=True):
        pass

    monkeypatch.setattr(sync_mod, "audit_inbound_hubspot_data", mock_audit)
    monkeypatch.setattr(sync_mod, "record_hubspot_sync_result", mock_record_result)
...
@pytest.mark.asyncio
async def test_sync_tenant_inbound_failure_records_error(monkeypatch):
    tenant_id = uuid.uuid4()
    ...
    res = await sync_tenant_inbound(session=DummySession(), tenant_id=tenant_id)

    assert res["status"] == "failed"
    assert recorded["status"] == "failed"
    assert "refresh token revoked" in recorded["error"]
