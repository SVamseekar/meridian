...
def test_verify_hubspot_v3_signature_missing_signature_or_timestamp():
    assert verify_hubspot_v3_signature(
        signature=None,
        timestamp=str(int(time.time() * 1000)),
        method="POST",
...
def test_verify_hubspot_v3_signature_falls_back_to_client_secret(monkeypatch):
    monkeypatch.delenv("HUBSPOT_WEBHOOK_SECRET", raising=False)
    secret = "client-secret-fallback"
    ...
    assert verify_hubspot_v3_signature(
        signature=signature, timestamp=timestamp, method=method, uri=uri, body=body
    ) is True
